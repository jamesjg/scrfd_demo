...
def add_files_in_folder_to_list(folder_path, imgs_path):
    ...
    for file in files:
        if file.endswith('.jpg') or file.endswith('.png'):
            assert os.path.exists(os.path.join(folder_path, file[:-4]+".mat")), f"Mat file not found for {file}"
            img_files.append(os.path.join(folder_path, file))

    imgs_path.extend(img_files)
...
if __name__ == '__main__':
    ...
    output_json_path = "face_detect_json_anno/300WLP_results.json"
    
    imgs_path = []
    subfolders = [f.path for f in os.scandir(folder_path) if f.is_dir() and f not in ['landmarks', 'Code']]
    for subfolder in subfolders:
        add_files_in_folder_to_list(os.path.join(folder_path, subfolder), imgs_path)

    print(f"Total images found: {len(imgs_path)}")
    results = []
    ...
    with open(output_json_path, 'w') as json_file:
        for img_path in imgs_path:
            ...
            img_name = img_path[len(folder_path)+1:] # remove the folder path and /
