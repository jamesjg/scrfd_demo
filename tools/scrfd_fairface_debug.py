...
def add_files_in_folder_to_list(folder_path, imgs_path):
    ...
    for subfolder in subfolders:
        ...
        file_paths = os.listdir(subfolder_path)
        file_paths.sort()
        file_paths = [os.path.join(subfolder_path, x) for x in file_paths if x.endswith(".jpg") or x.endswith(".png")]
        imgs_path.extend(file_paths)
    # file_paths = os.listdir(folder_path)
    # imgs_paths = [os.path.join(folder_path, x) for x in file_paths if x.endswith(".jpg") or x.endswith(".png")]
    # imgs_path.extend(imgs_paths)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='scrfd inference')
    parser.add_argument('--folder_path', type=str, default='/mnt/data/lanxing/fairface-1.25/', help='folder path')
    parser.add_argument('--output_json_path', type=str, default='face_detect_json_anno/debug/fairface_8.json', help='output json path')
    ...
    if os.path.isdir(args.folder_path):
        add_files_in_folder_to_list(args.folder_path, imgs_path)
    else:
    ...
    print(f"Detection results saved to {args.output_json_path}")