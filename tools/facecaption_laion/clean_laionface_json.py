...
def clean_laionface_json(json_path):
    ...
    save_dir = os.path.join(f"{os.path.dirname(json_path)}_refine", os.path.basename(json_path).replace('_new.json', '_clean'))   
...
root = 'face_detect_json_anno/laion_face'
json_files = [os.path.join(root, f) for f in os.listdir(root) if f.endswith('_new.json')]
print(json_files)
all_num_imgs_5_more_face = 0
for json_file in json_files:
    all_num_imgs_5_more_face+= clean_laionface_json(json_file)
    # exit()
