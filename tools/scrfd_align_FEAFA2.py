...
# 初始化 SCRFD 模型
    detector = SCRFD(model_file='/home/dingyan/huguohong/scrfd_demo-master/onnx/scrfd_10g_bnkps.onnx')
    detector.prepare(-1)

    # 输入文件夹路径和其他参数设置
    folder_path = r"/home/dingyan/huguohong/9_10/dataset/FEAFA_ALL/FEAFA_A1"
    output_dir = '/home/dingyan/huguohong/9_10/dataset/FEAFA_ALL/FEAFA_A1_align3'
    ...
    # 错误日志文件路径
    error_log_file = os.path.join(output_dir, 'error.txt')
    ...
    for img_file in os.listdir(folder_path):
        if img_file.lower().endswith(('.png', '.jpg', '.jpeg')):
            imgs_path.append(os.path.join(folder_path, img_file))
            
    imgs_path.sort()
    #print(imgs_path[:100])
    # 处理每个图像
    count = 0
    for img_path in imgs_path[:100]:
        ...
        if bboxes is not None and len(bboxes) == 1:
            ...
            with open(error_log_file, 'a') as f:
                f.write(f"More than 1 faces detected in {img_name}\n")
                
        else:
            with open(error_log_file, 'a') as f:
                f.write(f"No face detected in {img_name}\n")
        count += 1
        if count % 250 == 0:
            print(f'processed {count} imgs')
